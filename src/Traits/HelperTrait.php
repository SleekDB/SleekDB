  ...
  private function bootStore()
  {
    ...
    if (!file_exists($this->storePath)) {
      ...
      if (!mkdir($this->storePath, 0777, true)) {
        throw new IOException('Unable to create the store path at ' . $this->storePath);
      }
      // Create the cache directory.
      if (!mkdir($this->storePath . 'cache', 0777, true)) {
        throw new IOException('Unable to create the store\'s cache directory at ' . $this->storePath . 'cache');
      }
      // Create the data directory.
      if (!mkdir($this->storePath . 'data', 0777, true)) {
        throw new IOException('Unable to create the store\'s data directory at ' . $this->storePath . 'data');
      }
      // Create the store counter file.
      if (!file_put_contents($this->storePath . '_cnt.sdb', '0')) {
        throw new IOException('Unable to create the system counter for the store! Please check write permission');
      }
    }
    // Check if PHP has write permission in that directory.
    if (!is_writable($this->storePath)) {
      throw new IOException('Store path is not writable at "' . $this->storePath . '." Please change store path permission.');
    }
    // Finally check if the directory is readable by PHP.
    if (!is_readable($this->storePath)) {
      throw new IOException('Store path is not readable at "' . $this->storePath . '." Please change store path permission.');
    }
  }
